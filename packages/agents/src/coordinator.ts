...
import type { ConfigurationManager, GameState, ActionType } from "@poker-bot/shared";
import type { AgentModelConfig, JsonSchema } from "@poker-bot/shared/src/config/types";
import type {
  AgentCoordinator,
  AgentOutput,
  AgentQueryOptions,
...
export class AgentCoordinatorService implements AgentCoordinator {
  ...
  constructor(options: AgentCoordinatorOptions) {
    ...
    this.circuitBreaker = new CircuitBreaker(this.configManager.get("agents.circuitBreaker"));
  ...
  private buildPersonaTemplates(): Record<string, PersonaTemplate> {
    ...
    try {
      overrides = this.configManager.get<Record<string, unknown> | undefined>("agents.personaOverrides");
    } catch {
      overrides = undefined;
    }
    return createPersonaTemplates((overrides as Record<string, unknown>) ?? {});
  }
