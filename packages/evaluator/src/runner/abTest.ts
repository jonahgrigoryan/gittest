...
export class ABTestRunner implements EvaluationRunner {
  constructor(
    private readonly optionsA: HarnessOptions,
    private readonly optionsB: HarnessOptions,
  ) {}
  ...
  async run(context: EvaluationContext): Promise<EvaluationAggregateReport> {
    ...
    return {
      ...
      winRateConfidenceInterval: [
        reportA.winRateConfidenceInterval[0] -
          reportB.winRateConfidenceInterval[1],
        reportA.winRateConfidenceInterval[1] -
          reportB.winRateConfidenceInterval[0],
      ],
    };
  }
}
